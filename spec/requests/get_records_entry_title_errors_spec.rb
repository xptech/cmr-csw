...
describe "GetRecords Title error cases" do
  # No error, NO contraint CMR query
  it 'correctly renders default HITS data in response to the resultType missing and an incomplete Title constraint POST request' do
    VCR.use_cassette 'requests/get_records/gmi/title_error_literal', :decode_compressed_response => true, :record => :once do
...
<csw:GetRecords maxRecords="10" outputFormat="application/xml"
  ...
  it 'correctly renders default HITS data in response to the resultType missing Title POST request' do
    VCR.use_cassette 'requests/get_records/gmi/title_error_missing', :decode_compressed_response => true, :record => :once do
      get_records_request_xml = <<-eos
<csw:GetRecords maxRecords="10" outputFormat="application/xml"
    outputSchema="http://www.isotc211.org/2005/gmi" service="CSW"
    startPosition="1" version="2.0.2" xmlns="http://www.opengis.net/cat/csw/2.0.2"
  ...
  it 'correctly renders Exception page in response to an invalid XML POST request' do
    VCR.use_cassette 'requests/get_records/gmi/title_malformed', :decode_compressed_response => true, :record => :once do
      get_records_request_xml = <<-eos
<csw:GetRecords maxRecords="10" outputFormat="application/xml"
    outputSchema="http://www.isotc211.org/2005/gmi" service="CSW"
    startPosition="1" version="2.0.2" xmlns="http://www.opengis.net/cat/csw/2.0.2"
    ...
    <csw:Query typeNames="csw:Record">
      ...
      expect(exception_text.text).to eq("Could not parse the request body XML: Opening and ending tag mismatch: Constraint line 9 and ConstraintBAD_BAD_BAD")
    end
  end
end