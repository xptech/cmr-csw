...
RSpec.describe "various GetRecords POST requests based on spatial gml:Point criteria", :type => :request do

  it 'correctly renders FULL RESULTS ISO MENDS (GMI) data in response to a Point ONLY constraint POST request' do
    VCR.use_cassette 'requests/get_records/gmi/point_records_only', :decode_compressed_response => true, :record => :once do
...
<csw:GetRecords xmlns:csw="http://www.opengis.net/cat/csw/2.0.2" resultType="results"
    ...
    <csw:Query typeNames="csw:Record">
      ...
      expect(search_results_node_set[0]['numberOfRecordsMatched']).to eq('6616')
  ...
  it 'correctly renders BRIEF RESULTS ISO MENDS (GMI) data in response to a Point ONLY constraint POST request' do
    VCR.use_cassette 'requests/get_records/gmi/point_records_only', :decode_compressed_response => true, :record => :once do
      get_records_request_xml = <<-eos
<?xml version="1.0"?>
<csw:GetRecords xmlns:csw="http://www.opengis.net/cat/csw/2.0.2" resultType="results"
  ...
  it 'correctly renders BRIEF CSW RESULTS data in response to a Point ONLY constraint POST request' do
    VCR.use_cassette 'requests/get_records/gmi/point_records_only', :decode_compressed_response => true, :record => :once do
      get_records_request_xml = <<-eos
<csw:GetRecords maxRecords="10" outputFormat="application/xml"
  ...
  it 'correctly renders FULL RESULTS ISO MENDS (GMI) data in response to a AOI(Point)_TOI constraint POST request' do
    VCR.use_cassette 'requests/get_records/gmi/point_records_mixed', :decode_compressed_response => true, :record => :once do
      get_records_request_xml = <<-eos
<csw:GetRecords maxRecords="10" outputFormat="application/xml"
    ...
    <csw:Query typeNames="csw:Record">
      ...
      search_results_node_set = records_xml.root.xpath('/csw:GetRecordsResponse/csw:SearchResults', 'csw' => 'http://www.opengis.net/cat/csw/2.0.2')
      expect(search_results_node_set.size).to eq(1)
      expect(search_results_node_set[0]['numberOfRecordsMatched']).to eq('4598')
      expect(search_results_node_set[0]['numberOfRecordsReturned']).to eq('10')
      expect(search_results_node_set[0]['nextRecord']).to eq('11')
      expect(search_results_node_set[0]['elementSet']).to eq('full')
      expect(search_results_node_set[0]['recordSchema']).to eq('http://www.isotc211.org/2005/gmi')
  ...
  it 'correctly renders HITS data in response to a Point constraint POST request' do
    VCR.use_cassette 'requests/get_records/gmi/point_records_only', :decode_compressed_response => true, :record => :once do
      get_records_request_xml = <<-eos
<csw:GetRecords maxRecords="10" outputFormat="application/xml"
  ...
  it 'correctly renders default HITS data in response to the resultType missing and a Point constraint POST request' do
    VCR.use_cassette 'requests/get_records/gmi/point_records_only', :decode_compressed_response => true, :record => :once do
      get_records_request_xml = <<-eos
<csw:GetRecords maxRecords="10" outputFormat="application/xml"
