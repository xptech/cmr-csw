...
describe "GetRecords Instrument error cases" do
  # No error, NO contraint CMR query
  it 'correctly renders default HITS data in response to the resultType missing and an Instrument constraint with no literal POST request' do
    VCR.use_cassette 'requests/get_records/gmi/instrument_missing_literal', :decode_compressed_response => true, :record => :once do
...
<csw:GetRecords maxRecords="10" outputFormat="application/xml"
  ...
  it 'correctly renders Exception page in response to an invalid XML POST request' do
    VCR.use_cassette 'requests/get_records/gmi/instrument_malformed', :decode_compressed_response => true, :record => :once do
      get_records_request_xml = <<-eos
<csw:GetRecords maxRecords="10" outputFormat="application/xml"
    outputSchema="http://www.isotc211.org/2005/gmi" service="CSW"
    startPosition="1" version="2.0.2" xmlns="http://www.opengis.net/cat/csw/2.0.2"
