class OgcFilter
  ...
  def process_all_queryables
    @@SUPPORTED_QUERYABLE_PROCESSORS.each do |queryable_processor|
      ...
      Rails.logger.info "#{queryable_processor_instance.class.to_s} start queryable processing: #{@cmr_query_hash.to_s}"
      @cmr_query_hash.reverse_merge!(queryable_processor_instance.process(@ogc_filter))
      Rails.logger.info "#{queryable_processor_instance.class.to_s} end queryable processing: #{@cmr_query_hash.to_s}"
